...
namespace Leet::Medium {
    ...
    // same height in front, which is person 0 and 1. Person 3 has height 6 with
    // one person taller or the same height in front, which is person 1. Person
    // 4 has height 4 with four people taller or the same height in front, which
    // are people 0, 1, 2, and 3. Person 5 has height 7 with one person taller
    // or the same height in front, which is person 1. Hence
    ...
    struct QueueByHeight
    {
        vector<vector<int>> reconstructQueue(vector<vector<int>> &people)
        {
            // sort by decending height. So we know that person[i] is going to
            // be the shortest person in the list up to i. So if person[i] needs
            // i.e. 4 people infront, move person[i] 4 from the beginning
            ...
            for (auto person : people)
                order.insert(order.begin() + person[1], person);
