...
namespace Leet::Medium {
    ...
    // value at the given time timestamp. String get(String key, int timestamp) Returns a
    // value such that set was called previously, with timestamp_prev <= timestamp. If
    // there are multiple such values, it returns the value associated with the largest
    ...
    // timestamp = 4. timeMap.get("foo", 4);         // return "bar2" timeMap.get("foo",
    // 5);         // return "bar2"
    ...
    {
    private:
        unordered_map<string, vector<std::pair<int, string>>> store;

    public:
        TimeBasedStore() : store()
        {
        }
        ...
        {
            // if we don't use a reference here, we get TLE and MLE
            vector<std::pair<int, string>> &values = this->store[key];
            int l = 0;
            int r = values.size() - 1;
