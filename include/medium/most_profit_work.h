...
namespace Leet::Medium {
    ...
    // profit will be $3. If a worker cannot complete any job, their profit is $0. Return
    ...
    struct MostProfitWork
    {
        ...
        // this completes all tests but is not fast
        int maxProfitAssignment_slow(vector<int> &difficulty, vector<int> &profit,
                                     vector<int> &worker)
        {
            int total_profit = 0;
            int curr_profit = 0;
            ...
            for (int i = 0; i < worker.size(); i++) {
                ...
                for (int j = 0; j < difficulty.size(); j++) {
                    if (difficulty[j] <= worker[i])
                        curr_profit = std::max(curr_profit, profit[j]);
                }
