...
namespace Leet::Medium {
    ...
    // and triplets[j] = [1, 7, 5], triplets[j] will be updated to [max(2, 1), max(5, 7),
    // max(3, 5)] = [2, 7, 5]. Return true if it is possible to obtain the target triplet
    ...
    // [[2,5,3],[2,3,4],[2,5,5],[5,5,5]]. The target triplet [5,5,5] is now an element of
    // triplets.
        ...
        bool mergeTriplets(vector<vector<int>>& triplets, vector<int>& target)
        {
            ...
            for (vector<int>& t : triplets) {
                if (t[0] <= target[0] && t[1] <= target[1] && t[2] <= target[2])
                    ans = {std::max(t[0], ans[0]), std::max(t[1], ans[1]),
                           std::max(t[2], ans[2])};
            }
