...
namespace Leet::Medium {
    ...
    // 0, dst = 3, k = 1 Output: 700 Explanation: The graph is shown above. The optimal
    // path with at most 1 stop from city 0 to 3 is marked in red and has cost 100 + 600 =
    // 700. Note that the path through cities [0,1,2,3] is cheaper but is invalid because
    // it uses 2 stops.
    ...
    // 0 <= src, dst, k < n
    // src != dst
    struct CheapestFlightsKStops
    {
        // BFS mostly like dijkstra but we use a queue not a priority queue (ignoring the
        // stops too)
        int findCheapestPrice(int n, vector<vector<int>>& flights, int src, int dst,
                              int k)
        {
            vector<int> flightCost(n + 1, INT_MAX);
            ...
            while (stops <= k && !q.empty()) {
                ...
                while (size-- > 0) {
                    ...
                    for (auto& [v, edge] : graph[u]) {
                        if (cost + edge < flightCost[v]) {
                            flightCost[v] = cost + edge;
                            q.push({v, flightCost[v]});
                        }
                    }
                }
                stops++;
