...
namespace Leet::Medium {
    ...
    // only return x steps in the history and steps > x, you will return only x steps.
    // Return the current url after moving back in history at most steps.
    //
    // string forward(int steps) Move steps forward in history. If you can only forward
    // x steps in the history and steps > x, you will forward only x steps.
    ...
    //
    // browserHistory.back(1);                   // You are in "facebook.com", move back
    // to "google.com" return "google.com"
    //
    // browserHistory.forward(1);                // You are in "google.com", move forward
    ...
    {
    public:
        BrowserHistory(std::string homepage) : stack_ptr(0), stack()
        {
            stack.emplace_back(homepage);
        }

        void visit(std::string url)
        {
            stack.erase(stack.begin() + stack_ptr + 1, stack.end());
            stack.emplace_back(url);
            stack_ptr = stack.size() - 1;
        }

        std::string back(int steps)
        {
            stack_ptr -= std::min(stack_ptr, steps);
            return stack[stack_ptr];
        }

        std::string forward(int steps)
        {
            stack_ptr += std::min(steps, (int)stack.size() - stack_ptr - 1);
            return stack[stack_ptr];
        }
