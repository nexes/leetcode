...
namespace Leet::Medium {
    ...
    // digits in the guess that could be rearranged such that they become bulls. Given the
    // secret number secret and your friend's guess guess, return the hint for your
    // friend's guess.
    ...
    struct CowsBulls
    {
        string getHint(string secret, string guess)
        {
            ...
            for (int i = 0; i < guess.length(); i++) {
                if (auto found = freq.find(guess[i]); found != freq.end()) {
                    if (freq[guess[i]] > 0) {
                        cows++;
                        freq[guess[i]]--;
                    }
            ...
            hint = std::to_string(bulls) + "A" + std::to_string(cows) + "B";
            return hint;
        }
    };
}  // namespace Leet::Medium