...
namespace Leet::Medium {
    ...
    // lexicographically smallest one possible. Return the resulting string. If
    // there is no way to replace a character to make it not a palindrome,
    // return an empty string. A string a is lexicographically smaller than a
    // string b (of the same length) if in the first position where a and b
    // differ, a has a character strictly smaller than the corresponding
    // character in b. For example, "abcc" is lexicographically smaller than
    // "abcd" because the first position they differ is at the fourth character,
    ...
    struct BreakPalindrome
    {
        std::string breakPalindrome(std::string palindrome)
        {
            ...
            std::string break_pal = palindrome;
            ...
            break_pal[palindrome.length() - 1] = 'b';
            return break_pal;
        }
    };
}  // namespace Leet::Medium