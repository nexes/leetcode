...
namespace Leet::Medium {
    // Given two version numbers, version1 and version2, compare them. Version numbers
    // consist of one or more revisions joined by a dot '.'. Each revision consists of
    // digits and may contain leading zeros. Every revision contains at least one
    // character. Revisions are 0-indexed from left to right, with the leftmost revision
    ...
    // specify a revision at an index, then treat the revision as 0. For example,
    // version 1.0 is less than version 1.1 because their revision 0s are the same, but
    // their revision 1s are 0 and 1 respectively, and 0 < 1. Return the following:
    ...
    struct CompareVersions
    {
        int compareVersion(std::string version1, std::string version2)
        {
            ...
            // convert version1 string to list of ints
            for (int i = 0; i < version1.length(); i++) {
                if (version1[i] != '.') {
                    rev.push_back(version1[i]);
                    continue;
                }

                v1.push_back(std::atoi(rev.c_str()));
                rev = "";
            }
            v1.push_back(std::atoi(rev.c_str()));
            rev = "";

            // convert version2 string to list of ints
            for (int i = 0; i < version2.length(); i++) {
                if (version2[i] != '.') {
                    rev.push_back(version2[i]);
                    continue;
                }

                v2.push_back(std::atoi(rev.c_str()));
                rev = "";
            }
            v2.push_back(std::atoi(rev.c_str()));
