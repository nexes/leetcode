...
namespace Leet::Medium
{
    ...
    // to pay the transaction fee for each transaction. You may not buy more than 1
    // share of a stock at a time (ie. you must sell the stock share before you
    // buy again.) Return the maximum profit you can make.
    ...
    struct SellStock
    {
        int maxProfit(std::vector<int> &prices, int fee)
        {
            ...
            {
                profit = std::max(profit, h + prices.at(i) - fee);
                h = std::max(h, profit - prices.at(i));
            }
