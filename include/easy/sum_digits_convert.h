...
namespace Leet::Easy {
    ...
    // operation k times in total. For example, if s = "zbax" and k = 2, then the
    // resulting integer would be 8 by the following operations:
    ...
    struct SumDigitsStringConvert
    {
        int getLucky(std::string s, int k)
        {
            ...
            for (char c : s) {
                int t = (c - 'a') + 1;
                s_val.append(std::to_string(t));
            }

            auto reduce = [](std::string sval) -> std::string {
                int ival = 0;
                for (char c : sval) {
                    ival += (int)(c - '0');
                }
