...
namespace Leet::Easy {
    ...
    struct RemoveDuplicates
    {
        int removeDuplicates(std::vector<int>& nums)
        {
            ...
            // if nums is an empty array, just return 0
            if (size == 0)
                return 0;

            // since the array is sorted, just keep looking for the next element
            // larger than nums[insertIdx] and when we find it, update the value
            // this will result in some unneeded copies, but thats okay I think for this.
