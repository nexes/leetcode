...
namespace Leet::Easy {
    ...
    //
    // 0 <= i <= s.length - 2 s[i] is a lower-case letter and
    // s[i + 1] is the same letter but in upper-case or vice-versa.
    //
    // To make the string
    // good, you can choose two adjacent characters that make the string bad and remove
    // them. You can keep doing this until the string becomes good. Return the string
    ...
    struct StringGreat
    {
        ...
        {
            int sp = 0;
            std::string str_stack(s.length(), ' ');

            for (int i = 0; i < s.length(); i++) {
                if (sp > 0 && std::abs(str_stack[sp - 1] - s[i]) == 32)
                    sp--;
                else
                    str_stack[sp++] = s[i];
            }

            return str_stack.substr(0, sp);
        }
    };
}  // namespace Leet::Easy