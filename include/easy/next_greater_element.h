...
namespace Leet::Easy {
    ...
    // nums1.length, find the index j such that nums1[i] == nums2[j] and determine the
    // next greater element of nums2[j] in nums2. If there is no next greater element,
    ...
    struct NextGreaterElem
    {
        // naive solution
        // time: O(n*m)
        // space: O(1)
        vector<int> nextGreaterElement_naive(vector<int> &nums1, vector<int> &nums2)
        {
            vector<int> values(nums1.size(), -1);
        ...
        // time: O(n*m)
        vector<int> nextGreaterElement_map(vector<int> &nums1, vector<int> &nums2)
        {
