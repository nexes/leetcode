...
namespace Leet::Easy {
    ...
    struct SquareSortedArray
    {
        // trivial solution O(nlog(n))
        vector<int> sortedSquares_naive(vector<int> &nums)
        {
            vector<int> sorted;
        ...
        // two pointer solution: O(n) solution
        vector<int> sortedSquares(vector<int> &nums)
        {
            ...
            while (l <= r) {
                int l_squared = nums[l] * nums[l];
                int r_squared = nums[r] * nums[r];

                if (l_squared >= r_squared) {
                    sorted[idx--] = l_squared;
                    l++;
                } else {
                    sorted[idx--] = r_squared;
                    r--;
                }
            }
