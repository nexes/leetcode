...
namespace Leet::Easy {
    ...
    // linked lists begins. For example, the following two linked lists: begin
    ...
    // skipB = 1 Output: Reference of the node with value = 2 Input Explanation:
    // The intersected node's value is 2 (note that this must not be 0 if the
    // two lists intersect). From the head of A, it reads as [1,9,1,2,4]. From
    // the head of B, it reads as [3,2,4]. There are 3 nodes before the
    ...
    // = 2 Output: null Input Explanation: From the head of A, it reads as
    // [2,6,4]. From the head of B, it reads as [1,5]. Since the two lists do
    ...
    // structure. Each value on each linked list is in the range [1, 10^9]. Your
